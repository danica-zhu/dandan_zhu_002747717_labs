...
public class DataGenerator {
    ...
    private void generateCommentFile() throws IOException{
        //generate Order file
        try {
            file = new File(COMMENT_FILE_PATH);//只是对象的名字，不是file本身
            if(file.exists()){
                file.delete();
            }
            file.createNewFile();
            System.out.println("New Comment File Created");
            writer = new FileWriter(file);// 对file类的实例进行操作而不是文件本身
            ...
            try {
                writer.flush();// 不用每次都写入磁盘，优化性能；方便error检查和处理
                writer.close();
            } catch (IOException e) {
...
//            String column = userId+","+firstName+","+lastName+","+rating;
            String column = String.join(",",String.valueOf(userId), firstName, 
                    lastName, String.valueOf(rating));
